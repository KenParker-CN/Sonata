...
function QueueRow({
  ...
  return (
    <div
      ref={rowRef}
      className={cn(
        'group flex items-center gap-2 rounded-xl border-l-2 px-3 py-2.5 transition-colors',
        isCurrent ? 'border-primary bg-primary/10' : 'border-transparent hover:bg-accent/60',
      )}
    >
      ...
      >
        {/* Cover */}
        <div className="h-11 w-11 shrink-0 overflow-hidden rounded-lg bg-muted shadow-sm">
          {track?.cover ? (
...
export default function QueueList({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      {showHeader && (
        <div className="flex shrink-0 items-center justify-between gap-3 border-b border-border bg-surface-elevated px-4 py-4">
          <div className="flex items-center gap-2 min-w-0">
            <ListMusic size={17} className="shrink-0 text-primary" />
            <div className="min-w-0">
              <h2 className="text-sm font-semibold text-foreground">Queue</h2>
              <p className="text-[11px] text-muted-foreground">{upNextCount} {upNextCount === 1 ? 'track' : 'tracks'} up next</p>
            </div>
          </div>
